...
export default function UserMenu({ user, avatarUrl }: UserMenuProps) {
  if (!user) {
    return (
      <div className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-[var(--border)] bg-[var(--surface)] text-[var(--muted)]">
      ...
      <Link href="/perfil" aria-label="Mi perfil">
        {avatarUrl ? (
          <Image
            ...
            width={32}
            height={32}
            className="rounded-full border-2 border-[var(--border)] object-cover hover:opacity-80 transition-opacity"
